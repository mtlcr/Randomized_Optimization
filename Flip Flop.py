...
for bit_length in bit_length_options:
    ...
    print(bit_length)
    problem = mlrose.DiscreteOpt(length=bit_length, fitness_fn=mlrose.FlipFlop(), maximize=True, max_val=2)

    start_mark = time.time()
    RHC_curve = mlrose.random_hill_climb(problem, max_attempts=bit_length , curve=True)
    end_mark = time.time()
    ...
    # good performing GA
    start_mark = time.time()
    GA_curve = mlrose.genetic_alg(problem,pop_size=5 * bit_length,mutation_prob=0.1, max_attempts= 5 * bit_length, curve= True)
    end_mark = time.time()
    ...
    start_mark = time.time()
    SA_curve = mlrose.simulated_annealing(problem, schedule=mlrose.GeomDecay(decay=0.99 ), max_attempts= 10, max_iters= 1000, curve=True)
    end_mark = time.time()
...
# good mmic
    ...
    # fast mimic
    # start_mark = time.time()
    # MIMIC_curve = mlrose.mimic(problem, pop_size=1, max_attempts=5  , curve=True)
    # end_mark = time.time()
    # MIMIC.append([bit_length, end_mark - start_mark, MIMIC_curve[2][-1, 0], MIMIC_curve[2][-1, 1],MIMIC_curve[2][-1, 1] / (end_mark - start_mark)])
    # print("MIMIC",end_mark - start_mark)
