...
for bit_length in bit_length_options:
    print(bit_length)
    problem = mlrose.DiscreteOpt(length=bit_length, fitness_fn=mlrose.OneMax(), maximize=True, max_val=2)
    ...
    print("GA",end_mark - start_mark)

    # fast GA for runtime
    # start_mark = time.time()
    # GA_curve = mlrose.genetic_alg(problem, pop_size=10, mutation_prob=0.1, max_attempts= 10, curve= True)
    # end_mark = time.time()
    # geneticc_algm.append([bit_length, end_mark - start_mark, GA_curve[2][-1, 0], GA_curve[2][-1, 1], GA_curve[2][-1, 1] / (end_mark - start_mark)])

    start_mark = time.time()
    SA_curve = mlrose.simulated_annealing(problem, schedule=mlrose.GeomDecay(decay=0.99),max_attempts= bit_length*10,  curve=True)
    end_mark = time.time()
    sim_annealing.append([bit_length, end_mark - start_mark, SA_curve[2][-1, 0], SA_curve[2][-1, 1], SA_curve[2][-1, 1] / (end_mark - start_mark)])
    print("SA",end_mark - start_mark)

# mimic good
    start_mark = time.time()
    MIMIC_curve = mlrose.mimic(problem, pop_size=200, max_attempts=  bit_length, curve=True)
    end_mark = time.time()
    MIMIC.append([bit_length, end_mark - start_mark, MIMIC_curve[2][-1, 0], MIMIC_curve[2][-1, 1],MIMIC_curve[2][-1, 1] / (end_mark - start_mark)])
    print("MIMIC",end_mark - start_mark)

    # mimic fast
    # start_mark = time.time()
    # MIMIC_curve = mlrose.mimic(problem, pop_size=1, max_attempts=  1, curve=True)
    # end_mark = time.time()
    # MIMIC.append([bit_length, end_mark - start_mark, MIMIC_curve[2][-1, 0], MIMIC_curve[2][-1, 1],MIMIC_curve[2][-1, 1] / (end_mark - start_mark)])
    # print("MIMIC",end_mark - start_mark)
...
for i in range(1,4):
    ...
    df.plot(marker='o',xlabel=metrics[0],ylabel=metrics[i],
            title=metrics[i] + " vs " + metrics[0],figsize=(10,7))
